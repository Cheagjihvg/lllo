...
export const Account: React.FC<AccountProps> = ({ user, setUser }) => {
  ...
  const handleRedeemKey = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setUser({ ...user, plan: 'pro' }); // Update the user plan on success
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      ...
      <div
        className={`p-6 rounded-lg ${isDark ? 'bg-gray-800/50 backdrop-blur-sm' : 'bg-white'} shadow-lg border ${isDark ? 'border-blue-500/20' : 'border-gray-200'}`}
      >
